...
from .agentic_config import ZeroAgenticConfig
...
class ZeroAgenticBlock(nn.Module):
    """
    A single block of the zero Agentic architecture.
    Combines FlashSSM and GatedLinearAttention with a learned mix gate.
    """
    def __init__(self, config: ZeroAgenticConfig):
        super().__init__()
...
class ZeroAgenticModel(BaseModel):
    """
    zero Agentic Model — recursive inference with self-critique and persistent memory.
    Merged from the former IGRIS architecture into the zero model family as an
    agentic sub-variant.
    """
    def __init__(self, config: ZeroAgenticConfig):
        ...
        logger.info("zero_agentic", f"Initializing zero Agentic ({config.hidden_size} dim, {config.num_layers} layers)")

        # 1. Base Components
        self.token_embedding = nn.Embedding(config.vocab_size, config.hidden_size)
        self.pos_embedding = nn.Embedding(config.context_length, config.hidden_size)
        self.blocks = nn.ModuleList([ZeroAgenticBlock(config) for _ in range(config.num_layers)])
    ...
    def forward(
        ...
        # Memory vault bias: gives memory_vault a gradient path
        ...
        if labels is not None:
            ...
            # Reward thinking (subtract ponder cost) — fixing the sign bug from IGRIS
